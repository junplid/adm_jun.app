...
function Content({
          ...
          <div className="flex mt-3 flex-col pb-3 px-3 gap-y-1.5">
            {!isAddItem
              ? [
                  ...
                  <Button colorPalette={"red"} fontWeight={"bold"}>
                    Fechar Conta
                  </Button>,
                ]
              : [
  ...
  return (
    <>
      <DialogBody className="grid grid-cols-[minmax(220px,1fr)_1fr] gap-x-3">
        <div className="flex flex-col gap-y-1">items na mesa + total</div>

        <div className="relative flex flex-col gap-y-1 h-[calc(100vh-170px)] overflow-y-auto">
          <div className="h-full flex-1 pb-12.5 flex flex-col gap-y-3">
            {products.items_with_category.map((cat) => (
              <div key={cat.uuid}>
                <span>{cat.name}</span>
                <div className="grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] min-w-60 gap-1.5 auto-rows-[90px]">
                  {cat.items.map((item) => (
                    <div
                      key={item.uuid}
                      onClick={() => {
                      ...
                      className="flex flex-col bg-amber-50/5 p-3! py-2! rounded-md"
                    >
                      <div
                        style={{ opacity: item.view ? 1 : 0.4 }}
