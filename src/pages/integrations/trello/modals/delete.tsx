...
import { useDeleteTrelloIntegration } from "../../../../hooks/trelloIntegration";
...
export const ModalDeleteAgentAI: React.FC<PropsModalDelete> = (
  ...
  const onDelete = useCallback(async (): Promise<void> => {
    try {
      if (props.data?.id) await deleteTrello(props.data?.id);
    } catch (error) {
  ...
  return (
    <DialogContent w={"370px"}>
      <DialogHeader flexDirection={"column"} gap={0}>
        <DialogTitle>Deletar integração do trello</DialogTitle>
      ...
      <DialogBody>
        <div className="flex flex-col gap-y-1.5">
          ...
          <p>Fluxos de conversa que usam essa integração serão afetados.</p>
        </div>
      </DialogBody>
      <DialogFooter>
        <DialogActionTrigger>
