      ...
      {data.fields?.includes("payment_method") && (
        <Field label="Salvar METODO DE PAGAMENTO">
          <SelectVariables
            ...
            filter={(opt) => opt.filter((s) => s.type === "dynamics")}
            onCreate={(tag) => {
              updateNode(id, { data: { ...data, varId_save_payment_method: tag.id } });
            }}
            value={data.varId_save_payment_method}
            onChange={(e: any) => {
              updateNode(id, { data: { ...data, varId_save_payment_method: e.value } });
            }}
          />
        </Field>
      )}
      ...
      {data.fields?.includes("delivery_address") && (
        <Field label="Salvar ENDEREÇO DE ENTREGA">
          <SelectVariables
            ...
            filter={(opt) => opt.filter((s) => s.type === "dynamics")}
            onCreate={(tag) => {
              updateNode(id, { data: { ...data, varId_save_delivery_address: tag.id } });
            }}
            value={data.varId_save_delivery_address}
            onChange={(e: any) => {
              updateNode(id, { data: { ...data, varId_save_delivery_address: e.value } });
            }}
          />
        </Field>
      )}
      ...
      {data.fields?.includes("number_contact") && (
        <Field label="Salvar NUMERO DO LEAD">
          <SelectVariables
            ...
            filter={(opt) => opt.filter((s) => s.type === "dynamics")}
            onCreate={(tag) => {
              updateNode(id, { data: { ...data, varId_save_number_contact: tag.id } });
            }}
            value={data.varId_save_number_contact}
            onChange={(e: any) => {
              updateNode(id, { data: { ...data, varId_save_number_contact: e.value } });
            }}
          />
        </Field>
      )}
