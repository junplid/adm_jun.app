...
export const NodeSendFile: React.FC<Node> = ({ id }) => {
  const {
    options,
    actions,
    reactflow: {
  ...
  useEffect(() => {
    if (!options.staticFile.length && isFirst) {
      actions.getStaticFile("file");
      setIsFirst(true);
    }
  }, [options.staticFile.length, isFirst]);
          ...
          <div className="flex flex-col gap-y-2 bg-slate-300/10 p-1">
            {!file && (
              <div className="flex gap-1 bg-yellow-600/60 p-1">
                ...
                >
                  Faça o upload do novo arquivo abaixo para salvar no banco de
                  dados
                </span>
              </div>
            )}
