...
import { AiOutlineFieldTime, AiOutlineFilePdf } from "react-icons/ai";
...
export const NodeSendPdf: React.FC<Node> = ({ id }) => {
  const {
    options,
    actions,
    reactflow: {
  ...
  useEffect(() => {
    if (!options.staticPdf.length && isFirst) {
      actions.getStaticFile("pdf");
      setIsFirst(true);
    }
  }, [options.staticPdf.length, isFirst]);
      ...
      <div>
        <div className="mt-2 flex flex-col gap-2 p-2">
          <label className="nodrag flex flex-col gap-y-1">
            <span className="font-semibold text-white/80">Selecione o pdf</span>
            <SelectComponent
              styles={{
                valueContainer: {
                  paddingLeft: 9,
              ...
              options={options.staticPdf.map((st) => ({
                label: st.name,
                value: st.id,
              }))}
              isMulti={false}
              noOptionsMessage="Nenhum pdf encontrado"
              placeholder="Selecione o pdf*"
              value={
                data.staticFileId
                  ? {
                      label:
                        options.staticPdf.find(
          ...
          <div className="flex flex-col gap-y-2 bg-slate-300/10 p-1">
            {!file && (
              <div className="flex gap-1 bg-yellow-600/60 p-1">
                ...
                >
                  Faça o upload do novo pdf abaixo para salvar no banco de dados
                </span>
              </div>
            )}
            {file && (
            ...
            <Button
              ...
              onClick={async () => {
                if (file) {
                  ...
                  actions.createStaticFile("pdf", file, (newFile) => {
                    setFile(null);
                    setNodes((nodes) =>
                      nodes.map((node) => {
                        if (node.id === id) {
            ...
            >
              <span className="w-full flex-wrap" style={{ fontSize: 10 }}>
                {file ? "Salvar novo pdf" : "Carregar pdf"}
              </span>
            </Button>
          </div>
          {data.message?.length ? (
            ...
            <AutoCompleteTextarea
              ...
              setValue={(value) => {
                setNodes((nodes) =>
                  nodes.map((node) => {
                    if (node.id === id) {
                      node.data = { ...node.data, message: value } as DataNode;
                    }
                    return node;
                  })
                );
