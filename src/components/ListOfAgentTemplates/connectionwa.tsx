...
export function ConnectWAComponent(props: IProps) {
  ...
  const requestQRcode = useCallback(() => {
    setIsRequest(true);
    setQrCode(null);
    socket.emit("revoke-session", { connectionWhatsId: props.id });
    setTimeout(() => {
      socket.emit("create-session", { connectionWhatsId: props.id });
    }, 3000);
  }, [socket, props.id]);

  useEffect(() => {
    socket.on(`status-session-${props.id}`, (state?: "open" | "connecting") => {
      if (state && state === "open") setConnected(true);
    });
    socket.on(`qr-code-${props.id}`, (data: string) =>
      setQrCode({ v: data, valid: true }),
    );

    return () => {
      socket.off(`status-session-${props.id}`);
      socket.off(`qr-code-${props.id}`);
    };
  }, []);

  if (connected) {
    return (
      <div className="flex-1 bg-green-100/5 flex flex-col items-center justify-center gap-3 p-6 text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-sm">
          Sua automação está ativa e operando normalmente neste momento. O
          servidor está processando as execuções em tempo real.
        </p>

        <Button onClick={() => props.onConnected()} size="sm" className="mt-2">
          Fechar
        </Button>
      </div>
    );
  ...
  return (
    <>
      <div className="items-start flex mx-auto w-full max-[600px]:flex-col max-[600px]:items-center gap-2 gap-y-5 max-w-2xl">
        <div className="w-full flex items-baseline flex-col gap-y-5">
          <ul className="flex flex-col gap-y-3 text-white/75 max-[600px]:w-full">
            <li className="flex items-baseline max-[600px]:justify-center gap-x-3">
              <span>1.</span>
              <p>Abra o WhatsApp no seu celular.</p>
            </li>
            <li className="flex items-baseline gap-x-3 max-[600px]:justify-center">
              <span>2.</span>
              <p className="">
        ...
        <div className="mb-3">
          ...
          {!isRequest && (
            <div
              ...
              className="flex flex-col select-none text-white/70 gap-y-3 bg-white/5 items-center justify-center"
              style={{ width: "300px", height: "300px" }}
            >
              <BsQrCode size={30} />
              <span>Clique para gerar o QR Code</span>
            </div>
          )}
          {qrCode && (
